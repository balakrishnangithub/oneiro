...
void main() {
  ...
  group('VaultCrypto', () {
    test(
      'create produces a spec-shaped descriptor and unlocks itself',
      () async {
    ...
    test('JSON round-trip preserves the payload exactly', () async {
      final (_, crypto) = await VaultCrypto.create(
        'pw',
        salt: fixedSalt,
        kdfN: 256,
      );
    ...
    test('tampering with the ciphertext fails authentication', () async {
      ...
      final tampered = Uint8List.fromList(
        utf8.encode(jsonEncode({...envelope, 'ct': base64Encode(ct)})),
      );
      await expectLater(
    ...
    test('tampering with the nonce fails authentication', () async {
      ...
      nonce[0] ^= 0x01;
      final tampered = Uint8List.fromList(
        utf8.encode(jsonEncode({...envelope, 'nonce': base64Encode(nonce)})),
      );
      await expectLater(
        crypto.decryptJson(tampered),
        throwsA(isA<VaultAuthenticationException>()),
